...
public class Tram {
	...
	public JSONObject personStationDAO (JSONObject JsonRecu,Connection c){
		JSONObject obj=new JSONObject();
		if (JsonRecu.get("demandType").equals("PERSON_STATION_INDICATOR")) {
			...
			try {
				PreparedStatement stmt1 = c.prepareStatement("select sum(qte_pers) as nombre_personne_position, position , qte_pers from frequentation_station_tram group by (position,qte_pers);"); 
				ResultSet rs2 = stmt1.executeQuery();
				System.out.println("requête exécutée)"); 
				...
				while (rs2.next()) {
					...
					// Mapping de la classe CarIndicator (passage des résultats de la BDD en un objet java grâce au resultset 
					PersonStationIndicator personStation = new PersonStationIndicator (0 ,rs2.getString("position"),rs2.getInt("qte_pers"),0, null, null,rs2.getInt("nombre_personne_position"));
					
					System.out.println("récuperation des résultats du select"); 
					JSONObject personStationJSON = personStation.convertToJSON();
					// adding each sensor to the list already created
					listPersonStation.add(personStationJSON);
